...
def emit(g, data):
    ...
    card("ข่าไม่ใช่ขิง — ของที่หน้าตาคล้ายแต่ไม่เหมือน", "Galangal is not ginger — the look-alikes",
         bi("ข่า (khàa) เผ็ดซ่าเหมือนสน ใช้ในต้มข่า ต้มยำ และพริกแกง · ขิง (khǐng) หวานร้อน ใช้ผัดและของหวาน — สองอย่างนี้แทนกันไม่ได้ · มะกรูด (má-krùut) ใช้ใบฉีกและผิวขูด ไม่ใช้น้ำ ส่วน มะนาว (má-naao) คือมะนาวที่บีบ · โหระพาสามชนิด: กะเพรา (kà-phrao) ใบขนเผ็ดร้อน สำหรับผัดกะเพรา · โหระพา (hǒo-rá-phaa) หอมชะเอม ใส่แกงเขียวหวาน · แมงลัก (maeng-lák) กลิ่นมะนาว ใส่ขนมจีนน้ำยา",
            "Galangal (ข่า, khàa) is piny and sharp — tom kha, tom yam, every curry paste; ginger (ขิง, khǐng) is sweet-hot — stir-fries and sweets; they do not swap. Kaffir lime (มะกรูด, má-krùut) gives its torn leaf and grated zest, never its juice; the lime you squeeze is มะนาว (má-naao). Three basils: holy basil (กะเพรา, kà-phrao) — furred, peppery, for phat kaphrao; Thai sweet basil (โหระพา, hǒo-rá-phaa) — anise-scented, into green curry; lemon basil (แมงลัก, maeng-lák) — into khanom chin nam ya."),
         bi("ตะไคร้ (tà-khrái) ใช้แต่โคนขาว · ขมิ้น (khà-mîn) ทำให้แกงเหนือเหลือง · รากผักชี (râak phàk-chii) คือกลิ่นลับในพริกแกงและน้ำจิ้ม — ของสามอย่างที่คนเรียนมักไม่เคยเห็นทั้งต้น",
            "Lemongrass (ตะไคร้, tà-khrái): only the pale base. Turmeric (ขมิ้น, khà-mîn) is what makes a Northern curry yellow. Coriander root (รากผักชี, râak phàk-chii) is the hidden scent in pastes and dipping sauces — three things a learner has usually never seen whole."))
    ...
    card("กระทะกับไฟ — ผัด", "The wok and the fire — stir-frying",
         ...
         bi("คลาสส่วนใหญ่ใช้เตาแก๊สแรงแบบบ้านหรือร้าน ไม่ใช่เตาเหล็กร้านข้างทาง — ที่บ้านของคุณก็ทำได้ ขอแค่กระทะร้อนพอและอย่าใส่ของเยอะเกินไปในครั้งเดียว",
            "Most classes cook on ordinary strong gas rings, not a street stall's jet burner — which is the point: you can do this at home, if the pan is hot enough and you never crowd it."))
    ...
    body = (
        ...
        f'<p class="ck-note">{bi("เรียงตามชื่อ ไม่จัดอันดับ — ☎ LINE 🌐 คือช่องทางที่สารบัญมี · 📋 = อยู่บนกระดานด้านบน ที่เหลือยังไม่มีคำของตัวเองให้จด ถามก่อนไป · โรงเรียนไหนยังไม่มีเบอร์ ช่วยกันเติมได้", "Alphabetical, unranked — ☎ LINE 🌐 mark the channels the directory holds · 📋 = on the board above; the rest have stated nothing yet to write down, so ask first · a school with no contact can be filled in by anyone")}</p>'
        f'{schools_html}'
